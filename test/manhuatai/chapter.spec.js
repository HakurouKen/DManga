...
describe('Manhuatai', () => {
  describe('Chapter#download', function () {
    ...
    it('should download the manga chapter', async function () {
      ...
      fs.existsSync(path.join(TEST_DIR, '1.jpg')).should.be.true();
      // check the last image
      fs.existsSync(path.join(TEST_DIR, '8.jpg')).should.be.true();
      // should not contain file that not belongs to this chapter
      fs.existsSync(path.join(TEST_DIR, '9.jpg')).should.be.false();
    });
  });
});
