...
export default class DmzjManga {
  ...
  async getInfo(): Promise<MangaInfo> {
    ...
    return {
      ...
      chapters: $.find('.cartoon_online_border li a').toArray().map(el => getChapterInfoFromAnchor(el, DOMAIN)),
      otherVersions: $versionTitles.toArray().map((title, i) => {
        const subtitle = cheerio(title).text().trim().split('：')[1] || (i + 2);
        return {
          name: `${name} ${subtitle}`,
          chapters: $versionContents.eq(i).find('li a').toArray().map(el => getChapterInfoFromAnchor(el, DOMAIN)),
        };
      }),
    };
  }
