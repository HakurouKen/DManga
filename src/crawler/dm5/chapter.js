...
const CID_REGEXP = /\/m(\d+)\/?$/;

class Dm5Chapter extends Chapter {
  ...
  total() {
    const COUNT_REGEXP = /DM5_IMAGE_COUNT\s?=\s?(\d+)/;
      ...
      for (let script of scripts) {
        ...
        if (!$el.attr('src')) {
          ...
          if (COUNT_REGEXP.test(html)) {
            let count = +(html.match(COUNT_REGEXP) || [])[1];
            return count || 0;
          }
        }
      }
  ...
  _getImages(curPage = 1) {
    ...
    let baseUrl = `${pageUrl}${this.constructor.IMAGE_REQUEST_URL}?language=1&gtk=6&key=&cid=${this.cid}`;
...
Dm5Chapter.IMAGE_REQUEST_URL = 'chapterfun.ashx';

export default Dm5Chapter;
